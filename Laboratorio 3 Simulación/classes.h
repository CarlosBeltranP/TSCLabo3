...
enum parameters {ConstanteE,ConstanteA,ConstanteF}; //tendremos 3 paramatros como constantes A,E y f para este problema
...
class mesh{
        float parameters[3]; //la malla recibira ahpra 3 parametros E, A y f
        int sizes[4];
        node *node_list;
    ...
    public:
        void setParameters(float E,float A,float f){ // colocaremos nuestros 3 parametros definiendo cual es cada una
            parameters[ConstanteE]=E;
            parameters[ConstanteA]=A;
            parameters[ConstanteF]=f;
        }
