Matrix createLocalK(int element,mesh &m){
    ...
    float E = m.getParameter(ConstanteE), //declararemos nuestras 2 cosntantes (E,A) que se utilizan en la creacion de nuestra matriz k local
    A = m.getParameter(ConstanteA),
    l = m.getNode(element+1).getX() - m.getNode(element).getX();
    //Para este ejercicico nuestra matriz local K queda definida como nuestra constante -(E*A)/(2*l) por nuestra matriz 2x2 [1 -1]
                                                                                                                    //  [-1 1]
...
Vector createLocalb(int element,mesh &m){
    Vector b;
    float f = m.getParameter(ConstanteF),
    l = m.getNode(element+1).getX() - m.getNode(element).getX();
    // la b local queda definida por la constante (f*l)/2 por el vector 2x1 [1]
    //                                                                      [1]
    b.push_back(f*l/2);
    b.push_back(f*l/2);
...
void applyNeumann(mesh &m,Vector &b){
    for(int i=0;i<m.getSize(NEUMANN);i++){
        ...
        b.at(c.getNode1()-1) -= c.getValue(); //Restaremos el valor de neumann para  la solucion de este ejercicio ya que cuando
        //hacemos el despeje este se resta en lugar de sumarse
    }
}
